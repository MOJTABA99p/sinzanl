...
class Robot_Rubika:
	...
	def editname(self, first_name):
	    inData = {"method":"updateProfile","input":{"first_name":first_name,"last_name":"null","updated_parameters":["first_name","last_name"]},"client":SetClines.web}
	    while 1:
	        try:
	...
	def getContacts(self, user_guid):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Robot_Rubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("users")
	        except:continue
	def getInfoByUsername(self, username):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Robot_Rubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("user")
				break
			except: continue
